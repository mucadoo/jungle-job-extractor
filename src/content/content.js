...
function extractText() {
    ...
    const jobDetails = {
        ...
        salary: metadataBlock?.querySelector('i[name="salary"]')?.parentElement?.textContent.replace(/.*Salaire :/i, '').trim() || (jobData?.salary_min ? `${jobData.salary_min} ${jobData.salary_currency || ''}` : null),
        ...
        skills: jobData?.tools?.length ? jobData.tools.map(tool => tool.name).join(', ') : Array.from(metadataBlock?.querySelectorAll('div[variant="default"][length] span') || []).map(el => el.textContent.trim()).join(', '),
...
function showToast(message) {
    let toast = document.querySelector('.jje-toast');
    if (!toast) {
